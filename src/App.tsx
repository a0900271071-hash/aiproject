  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPlayers(prevPlayers => {
        ...
        const aiKillerUpdates: Record<string, {
          x: number;
          z: number;
          rotationY: number;
        ...
        // --- AI 殺手決策與攻擊結算 (針對所有 AI 殺手獨立運算，並確保狀態隔離) ---
        const aiKillers = prevPlayers.filter(p => p.faction === 'killer' && p.id !== humanPlayerId && p.health !== 'dead');
        for (const kPlayer of aiKillers) {
          const killerRes = updateKillerAI(kPlayer, aiCtx);
          aiKillerUpdates[kPlayer.id] = {
            x: killerRes.updatedKiller.x,
            z: killerRes.updatedKiller.z,
            rotationY: killerRes.updatedKiller.rotationY,
            attackCooldown: killerRes.updatedKiller.attackCooldown || 0,
            skillCooldown: killerRes.updatedKiller.skillCooldown,
            berserkTime: killerRes.updatedKiller.berserkTime || 0,
          };
        ...
        const updatedPlayers = prevPlayers.map(p => {
          ...
          if (p.id === humanPlayerId) {
            ...
            if (p.faction === 'killer') {
              const kUpdate = aiKillerUpdates[p.id];
              if (kUpdate) {
                nx = kUpdate.x;
                nz = kUpdate.z;
                rot = kUpdate.rotationY;
                attackCD = kUpdate.attackCooldown;
                skillCD = kUpdate.skillCooldown;
    ...
    <div className="relative w-full h-screen bg-slate-950 text-white overflow-hidden select-none">
      ...
      {gamePhase === 'menu' && (
        <MainMenu
          ...
          initialFaction={userFaction}
          initialCharacterId={userCharId}
          initialMap={selectedMap}
          onBackToTitle={() => setGamePhase('title')}
          onExitApp={() => setGamePhase('title')}
        />
      )}
      {gamePhase === 'playing' && (
        <>
